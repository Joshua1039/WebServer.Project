...
var pivotGridInstance = $("#pivot_Grid").dxPivotGrid({
    ...
    stateStoring: {//area 지정 상태 저장
        enabled: true,
        type: "localStorage",
        storageKey: "queueStts"//key는 파일명과 동일하게함
    },
    dataSource: {
        fields: [{
            caption: "월",
            ...
            summaryType: "avg",
	            calculateSummaryValue: function(e) { //HH:MM:SS로 표기 시 차트에러. 계산 후 표기는 customizeText 사용
	            	return parseInt(e.value("call_time") / e.value("CON_CALL"));
            },
            customizeText: function(e) {
           		var retTxt = customText(e);
           		return retTxt;
        ...
        store: new DevExpress.data.CustomStore({
        	load: function (loadOptions) {
        		var d = $.Deferred();
	  	       	  ...
	  	       	  $.getJSON('/MWTS/mctm/selectQueueStts.do', {
	  	   		    format: "json",
		...
		.dxChart({
	        ...
	        tooltip: {//마우스오버
	            ...
	            customizeTooltip: function(e) {
					...
					}else if(e.seriesName.indexOf("응대율") != -1){
						return {
...
$("#chartNewBtn").dxButton({
    ...
    onClick: function(e) {
		...
		if ((agent.indexOf("msie") != -1) || (navigator.appName == 'Netscape' && navigator.userAgent.search('Trident') != -1)) {
			...
			window.open("/MWTS/mctm/queueSttsChartIE.do");
		}else{
			//IE 브라우저 외
			pivotOption = $("#pivot_Grid").dxPivotGrid("instance").option();
			
			window.open("/MWTS/mctm/queueSttsChart.do");
		}
    }
});
</script>